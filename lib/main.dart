...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> displayPrediction(Prediction? p) async {
    if (p != null) {
      ...
      await _places.getDetailsByPlaceId(p.placeId!); // Use ! to assert non-null
      // var address = await Geocoder.local.findAddressesFromQuery(p.description);

      // update the state and update the values in shared preferences for persistence
      setState(() {
        _homeLocation = p.description!;
        _homeLatitude = detail.result?.geometry?.location.lat ?? 0.0;
        _homeLongitude = detail.result?.geometry?.location.lng ?? 0.0;
      });
  ...
  void _onGeofence(bg.GeofenceEvent event) {
    ...
    var androidPlatformChannelSpecifics = AndroidNotificationDetails(
      ...
      'channelName', // Required for Android
